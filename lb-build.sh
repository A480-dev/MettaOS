...
target_image_name() {
  ...
  if [ "$KALI_VARIANT" = "default" ]; then
    echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}metta-os-${METTA_VERSION}-$KALI_ARCH.$IMAGE_EXT"
  else
    echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}metta-os-${METTA_VERSION}-$KALI_VARIANT-$KALI_ARCH.$IMAGE_EXT"
  fi
}
...
failure() {
  echo "Build of METTA OS ($KALI_DIST/$KALI_VARIANT/$KALI_ARCH) failed (see build.log for details)" >&2
...
echo -e "\n***\nGENERATED METTA OS IMAGE: $(readlink -f $TARGET_DIR/$(target_image_name $KALI_ARCH))\n***"
